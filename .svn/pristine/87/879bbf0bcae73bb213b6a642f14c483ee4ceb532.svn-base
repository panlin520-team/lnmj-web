...
<div class="layui-fluid">
    <div class="demoTable layui-form">
        <div class="layui-form-item">
            <div class="layui-inline">
                <label class="layui-form-label resetLabel">名称</label>
    ...
    <table id="supplierCategory" lay-filter="demo"></table>
...
<#--添加-->
    <div class="layui-fluid" id="addeditformdivid" hidden="" style="margin: 15px;">
        <form class="layui-form layui-form-pane layui-personal" action="" id="addeditformid">
            <div class="layui-form-item">
                <label class="layui-form-label xrequired">供应商分类名称</label>
                <div class="layui-input-block">
                    <input type="text" name="SupplierCategoryName" lay-verify="required" autocomplete="off"
...
<#--修改-->
    <form class="layui-form layui-form-pane layui-personal" id="editForm" action="" lay-filter="exampleEdit" method="post" hidden="true">
        <div class="layui-form-item" hidden="hidden">
            <label class="layui-form-label">供应商分类id</label>
            <div class="layui-input-block">
                <input type="text" name="supplierCategoryId" lay-verify="required" autocomplete="off" placeholder=""
                       class="layui-input">
            </div>
        </div>
...
<script>
    ...
    layui.use('table', function () {
        ...
        table.render({
            elem: '#supplierCategory'
            , url: storeHost + "/manage/supplier/listSupplierCategory"
            , id: "supplierCategoryReload"
            ...
            , cols: [[
                ...
                , {field: 'supplierCategoryId', title: '供应商分类Id', sort: true, fixed: 'left'}
                , {field: 'supplierCategoryName', title: '供应商分类名称'}
    ...
    layui.use(['table', 'layer', 'jquery', 'form'], function () {
        ...
        form.on('submit(addeditsubmitfilter)', function (data) {
            ...
            $.ajax({
                type: "POST",
                url: storeHost + "/manage/supplier/addSupplierCategory",
                data: $('#addeditformid').serialize() + "&createOperator=" + "${currentUser.trueName!}"+ "&companyType=" + "${currentUser.companyType!}"+ "&companyId=" + "${currentUser.companyId!}",
                async: false,
                ...
                success: function (data) {
                    ...
                    if (data.responseStatusType.message == 'Success') {
                        ...
                        //重新加载表格
                        table.reload("supplierCategoryReload");
                    }
                }
            });
            return false;
    ...
    function layerDelete(data, table, $) {
        //删除数据库数据
        var url = storeHost + "/manage/supplier/deleteSupplierCategory";
        var data = {
    ...
    function layerShowAndEdit(form, data, table, $) {
        ...
        var a1 = layer.open({
            ...
            yes: function (index, layero) {
                var url = storeHost + "/manage/supplier/updateSupplierCategory";
                var params = {
                    supplierCategoryId: layero.find("input[name='supplierCategoryId']").val(),
                    supplierCategoryName: layero.find("input[name='supplierCategoryName']").val(),
                    modifyOperator: "${currentUser.trueName!}"
                };
