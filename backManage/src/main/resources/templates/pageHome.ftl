            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body layuiadmin-card-list">
                    ...
                    <p class="layuiadmin-big-font">
                        ...
                        <label style="font-size: 15px;">退款金额为：</label><a id="rechargeRefuse" class="layuiadmin-big-font"></a>
                    </p>
                    <p>
                        单位：元
                        <span class="layuiadmin-span-color"><i
...
<script type="text/javascript">
    ...
    function showPage(storeId) {
        var data = {}, orderPrice = [], allPayPrice = 0, cashPayPrice = 0, orderNumber, newVIP, noVIP, recharge = 0,rechargeRefuse = 0,
                yejiData = {},
                gongzi = {};
        //获取柱形图数据-订单
        $.ajax({
            ...
            method: "POST",
            data: {"storeId": storeId,"date": chooseDate},
            async: false,
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.responseStatusType.message == "Success") {
                    recharge = res.result.rechargeAmount;
                    rechargeRefuse = res.result.totalRefuse;
                }
            }
        });
        //获取显示的统计数据
        ...
        $("#allPayPrice").text(orderPrice == null ? 0 : orderPrice.total);
        $("#checkCashIncome").text(cashPayPrice);
        $("#orderNumber").text(orderNumber);
        $("#newVIP").text(newVIP);
        $("#noVIP").text(noVIP);
        $("#recharge").text(recharge);
        $("#rechargeRefuse").text(rechargeRefuse);
        //获取业绩柱状图数据
        $.ajax({
                ...
                layui.use('table', function () {
                    ...
                    table.render({
                        ...
                        , cols: [[
                            ...
                            , {fixed: 'right', field: 'createTime', title: '创建时间', width: 120, align: 'center'}
                        ]]
                    });
                });
            }
