...
<script>
        ...
        table.render({
            ...
            , done: function (res, curr, count) {
                if ("${currentUser.companyType!}" == 1) {
                    $("#subCompanySearchLabel").css("display", "block");//显示div
                    $("#subCompanySearchDiv").css("display", "block");//显示div
                }
    ...
    layui.use(['table', 'layer', 'jquery', 'form', 'element'], function () {
        ...
        var $ = layui.$, active = {
            search: function () {
                ...
                var subCompanyId = $("select[name='subCompany'] option:selected").val();
                table.render({
                    elem: '#department'
                    , url: storeHost + "/manage/department/listDepartment"
                    , id: "departmentReload"
    ...
    function loadSubCompanyStore(form) {
        ...
        $.post(url, data, function (result) {
            if (result.responseStatusType.message == "Success") {
                ...
                var server = document.getElementById("subCompany"); //server为select定义的id
                ...
                for (var p in list) {
                    ...
                    var storeVOList = list[p].subsidiaryList;
                    for (var n in storeVOList) {
                        var option = document.createElement("option");  // 创建添加option属性
                        option.setAttribute("value", storeVOList[n].subsidiaryId); // 给option的value添加值
                        option.setAttribute("data-orgK3Number", storeVOList[n].k3Number); // 给option的value添加值
                        option.innerText = storeVOList[n].subsidiaryName;     // 打印option对应的纯文本
    ...
    //初始化子公司分组列表
    function loadSubCompanyStoreAdd() {
        var url = storeHost + "/manage/company/selectCompanyAndSubListNoPage";
        var data = {companyId: "${currentUser.companyId!}", companyType: "${currentUser.companyType!}"};
        $.post(url, data, function (result) {
            if (result.responseStatusType.message == "Success") {
