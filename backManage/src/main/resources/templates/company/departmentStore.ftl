    ...
    <div class="layui-fluid" id="addeditformdivid" hidden="" style="margin: 15px;">
        <form class="layui-form layui-form-pane layui-personal" action="" id="addeditformid">
            <div class="layui-form-item">
                <label class="layui-form-label">请选择门店</label>
                <div class="layui-input-inline">
                    <select name="companyId" id="storesAdd" lay-filter="storesAdd">
                    </select>
                </div>
            </div>
            <div class="layui-form-item">
            ...
            <div class="layui-form-item" style="display: flex">
                <label class="layui-form-label xrequired" style="width: 210px">是否为退货默认部门</label>
                <div class="layui-input-block" style="margin-left:0; width: 283px">
                    <select name="isDefaultDept" lay-filter="isDefaultDept" lay-verify="required">
...
<script>
    ...
    layui.use(['table', 'layer', 'jquery', 'form', 'element'], function () {
        ...
        form.on('submit(addeditsubmitfilter)', function (data) {
            ...
            $.ajax({
                ...
                data: $('#addeditformid').serialize() + "&createOperator=" + "${currentUser.trueName!}" + "&companyType=3",
                async: false,
        ...
        var $ = layui.$, active = {
            search: function () {
                ...
                var companyId = $("select[name='stores'] option:selected").val();
                table.render({
                    elem: '#department'
                    , url: storeHost + "/manage/department/listDepartment"
                    , id: "departmentReload"
    ...
    //初始化子公司分组列表
    function loadSubCompanyStoreAdd(form) {
        var url = storeHost + "/manage/store/selectSubCompanyAndStoreNoPage";
        var data = {companyId: "${currentUser.companyId!}", companyType: "${currentUser.companyType!}"};
        $.post(url, data, function (result) {
            if (result.responseStatusType.message == "Success") {
