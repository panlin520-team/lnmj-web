    ...
    <script>
        ...
        layui.use(['layer', 'form', 'table'], function () {
            ...
            $("#choseIcon").click(function () {
                layer.open({
                    type: 2,
                    area: ['95%', '95%'],
                    content: '${basePath!}/page/systemSetting/icons.html',
                    shadeClose: true,
                    end: function () {
                        $("#icon").select();
                    }
        ...
        function pageInit() {
            ...
            function beforeEditName(treeId, treeNode) {
                ...
                setTimeout(function () {
                    if (confirm("进入节点 -- " + treeNode.name + " 的编辑状态吗？")) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: authHost + "/manager/menu/selectMenuById",
                            data: {id: treeNode.id},
                            async: false,
            ...
            function onRemove(e, treeId, treeNode) {
                ...
                $.ajax({
                    type: "POST",
                    url: authHost + "/manager/menu/deletemenu",
                    data: {id: treeNode.id},
                    async: false,
                    error: function (request) {
                        layer.alert("与服务器连接失败/(ㄒoㄒ)/~~");
            ...
            layui.form.on('submit(addeditsubmitfilter)', function (data) {
                ...
                $.ajax({
                    type: "POST",
                    url: authHost + "/manager/menu/addupdatemenu",
            ...
            $.ajax({
                type: "POST",
                url: authHost + '/manager/menu/loadCheckMenuInfo?parentId=-1',
                async: false,
                dataType: 'json',
                timeout: 1000,
                cache: false,
